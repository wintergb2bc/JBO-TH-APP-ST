...
import { Actions } from "react-native-router-flux";
 ...
 const slotSort = [
  ...
  },
  {
    title: "แจ็คพอต", // 独特的游戏
    categoryID: 55,
    imgUrl: require("./../images/slot/slotType5.png")
  }
];
 const gameData = [
   {
   ...
   },
   {
     title: "PG สล็อต",
     id: "PGS",
     imageSrc: require('../images/slot/Pg-Grey.png'),
     imageActiveSrc: require('../images/slot/PG-Green.png')
   },
   {
    title: "MGS สล็อต",
    id: "MGP",
    imageSrc: require('../images/slot/MGS-Grey.png'),
    imageActiveSrc: require('../images/slot/MGS-Green.png')
    },
   {
     title: "PP สล็อต", 
     id: "TG",
     imageSrc: require('../images/slot/PP-gray.png'),
     imageActiveSrc: require('../images/slot/PP-green.png'),
   },
   {
     title: "SG สล็อต", 
     id: "SPG",
     imageSrc: require('../images/slot/SG-Gray.png'),
     imageActiveSrc: require('../images/slot/SG-Green.png')
   },
   {
    title: "PNG สล็อต",
    id: "YDSPNG",
    imageSrc: require('../images/slot/PNG-Gray.png'),
    imageActiveSrc: require('../images/slot/PNG-Green.png')
   },
   {
     title: "RLX สล็อต",
     id: "YDSRLX",
     imageSrc: require('../images/slot/RLX-Gray.png'),
     imageActiveSrc: require('../images/slot/RLX-Green.png')
    },
    {
      title: "YD สล็อต",
      id: "YDS",
      imageSrc: require('../images/slot/YG-Gray.png'),
      imageActiveSrc: require('../images/slot/YG-Green.png')
    }
 ];
import { Tabs, Toast, Flex, SegmentedControl, Modal } from "antd-mobile-rn";
...
let GameDB;
class SlotGameList extends React.Component {
  constructor(props) {
    super(props);
  ...
  //跳轉
  navigateToScene(key, GameOpenUrl,gametypes,BlegameReH,playItem) {
    const {gametype} = this.state;
    NowGameTitle = gameData.filter(v=>gametype===v.id)[0].title || '';
    Actions.SlotGamePage({ GameOpenUrl: GameOpenUrl, gametype: gametype,playItem:playItem });
  }
  ...
  PlayGame(gameid, gametype, img, item) {
    ...
    if(gametype==="YDSPNG"){
      UMonEvent("Game", "Launch", `${item.gameName}_PNGSlot`);   //Piwki 追蹤 加薪遊戲 記得添加
    }else if(gametype==="YDSRLX"){
      UMonEvent("Game", "Launch", `${item.gameName}_RLXSlot`);   //Piwki 追蹤 加薪遊戲 記得添加
    } else if(gametype==="MGP"){
      UMonEvent("Game", "Launch", `${item.gameName}_MGSSlot`);
    } else {
      UMonEvent("Game", "Launch", "SlotGames_ProductPage");   //Piwki 追蹤 加薪遊戲 記得添加
    }
    Toast.loading("เริ่มล็อบบี้เกม...", 200);
    ...
    fetchRequest(ApiPort.Game + gameid + "?isDemo=false&", "POST", data)
      .then(data => {
        ...
        if (data.errorCode == 0) {
          ...
          this.navigateToScene("GamePage", data.gameUrl, gametypes,BlegameReH,item);
        } else {
          Toast.fail(
            "เรียนท่านสมาชิกเกม ที่คุณเปิดอยู่ระหว่างการปรับปรุง โปรดเข้าเกมอีกครั้งในภายหลัง หากคุณมีคำถามใด ๆ โปรดติดต่อแชทสดของเรา",
            //"亲爱的竞博会员您所打开的游戏正在维护中，请稍后再尝试进入。若您有任何疑问，请联系我们的在线客服。",
          ...
          {item.provider == "JBP" ||
            (item.provider == "JBR" && (
              <View style={styles.imgTextBox}>
                <Flex.Item style={{ flex: 0.7 }}>
                  <Text
                    style={{ color: "#fff", fontSize: 10, paddingLeft: 10 }}
                  >
                    {item.gameName
                      ? item.gameName.length > 9
                        ? item.gameName.substr(0, 9) + ".."
                ...
                <Flex.Item alignItems="flex-end" style={{ flex: 0.25 }}>
                  <Text
                    style={{ color: "#4ee42b", fontSize: 12, paddingRight: 5 }}
                  >
                    >
                  </Text>
                </Flex.Item>

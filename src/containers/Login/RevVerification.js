...
//import RevVerifyFail from "./RevVerifyFail";
...
let verificationType = {
  ...
  email: {
    ...
    txt6: "OTP ที่ส่งไปยังอีเมลของคุณ",
  },
};
...
class OtpVerification extends React.Component {
  constructor(props) {
    ...
    //手機驗證 > 默認直接進入SMS驗證頁面!!!
    console.log('otpType[0]',otpType.sms,'otpType[1]',otpType.voice,this.props.username)
  }

  componentWillMount() {
    this.setData();

    ////先拿缓存，判断是否过期,一開始進頁面 沒有資料！！
    this.getDownTime();
    ...
    if (this.state.verificaType == "phone") {
      ...
      global.storage
        ...
        .then((res) => {
          ...
          this.setState({ storagePHTime: res  });
          if (new Date().getTime() < res) {
            console.log('text 倒計時還沒結束！！')
            this.TimeDownPhone();
            this.setState({
              isTxt:true, 
              isVoice:false, 
  ...
  verifyTAC() {
    ...
    const data = {
      ...
      verificationType: "RevalidatePassword",
      otp: this.state.verificationCode,
    };

    if (verificaType == "phone") {
      // if(consequence == 0){
      //   UMonEvent("Verification", "Submit", "Verify_PhoneSMS_ResetPW");
      // }else{
      //   UMonEvent("Verification", "Submit", "Verify_PhoneVoice_ResetPW");
      // }
      UMonEvent("Verification", "Submit", "Verify_phone_ResetPW");
    } else {
      UMonEvent("Verification", "Submit", "Verify_Email_ResetPW");
    }
        ...
        if (res) {
          ...
          if (!res.isSuccess) {
            ...
            const temp =
              ...
              5;
            console.log(temp,'getCodePhone',getCodePhone,getCodeVoice);
            // this.props.updateVerifyTimes(5);
            if (res.remainingAttempts !== -1) {
              this.props.updateVerifyTimes(temp);
            }
            // this.setState({ toastErrorFlag: true, toastMsg: (verificaType == "phone"&&getCodePhone)||(verificaType == "email"&&getCodeEmail)?'验证码已失效，请重新获取新的验证码':res.message });
            ...
            // this.setState({ toastErrorFlag: true, toastMsg: (verificaType == "phone"&&getCodeVoice)||(verificaType == "phone"&&getCodePhone)||(verificaType == "email"&&getCodeEmail)?'รหัสหมดอายุ กรุณาคลิกส่งอีกครั้ง เพื่อรับรหัสใหม่':"รหัสไม่ถูกต้อง กรุณาตรวจสอบให้แน่ใจว่าคุณ ได้กรอกรหัสที่ถูกต้อง" });
            // setTimeout(() => {
            //   this.setState({ toastErrorFlag: false });
            // }, 3000);
            ...
            // this.setState({ toastSuccessFlag: true, toastMsg: '验证成功' });
            this.setState({ 
            ...
            //验证成功跳转
            console.log('验证成功跳转username========',this.props.username)
            Actions.Renewpassword({
              username:this.props.username
            })
  ...
  sendVerfication(type) {
    ...
    if (verificaType == "phone") {
      if(type == 0){
        UMonEvent("Verification", "Click", "Sendcode_SMS_phone_ResetPW");
      }else{
        UMonEvent("Verification", "Click", "Sendcode_Voice_phone_ResetPW");
      }
      // if(codeAgainPhone){
      //   if(type == 0){
      //     UMonEvent("Verification", "Click", "Resendcode_SMS_phone_ResetPW");
      //   }else{
      //     UMonEvent("Verification", "Click", "Resendcode_Voice_phone_ResetPW");
      //   }
      // }else{
      //   if(type == 0){
      //     UMonEvent("Verification", "Click", "Sendcode_SMS_phone_ResetPW");
      ...
      UMonEvent("Verification", "Click", "Sendcode_Email_ResetPW");
      // if(codeAgainEmail){
      //   UMonEvent("Verification", "Click", "Resendcode_Email_ResetPW");
      // }else{
      //   UMonEvent("Verification", "Click", "Sendcode_Email_ResetPW");
      // }
    }
  ...
  render() {
    ...
    return (
      <View style={{ flex: 1, backgroundColor: "#000000" }}>
        {/* {this.props.verifyLimitReached && <RevVerifyFail />} */}
        ...
        {!this.props.verifyLimitReached && (
          <KeyboardAwareScrollView resetScrollToCoords={{ x: 0, y: 0 }}>
            ...
            <Modal
              ...
              onRequestClose={() => { }}                
            >
              {/* <View style={{ zIndex: 10000, width:300, heigt:54, borderRadius:5, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', padding: 12, backgroundColor: "#FFFFFF", position:'absolute', top:Platform.OS === "android"?350:400, left:Platform.OS === "android"?50:50}}> */}

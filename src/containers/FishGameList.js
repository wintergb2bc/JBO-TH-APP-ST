...
import { Actions } from "react-native-router-flux";
 ...
 const gameData = [
   ...
   },
   {
     title: "JL ยิงปลา​",
     id: "JIF",
     imageSrc: require('../images/slot/JIF-Gray.png'),
     imageActiveSrc: require('../images/slot/JIF-Green.png')
   },
   {
    title: "SG ยิงปลา​",
    id: "SPG",
    imageSrc: require('../images/slot/SG-Gray.png'),
    imageActiveSrc: require('../images/slot/SG-Gray.png')
    },
   
 ];
import { Tabs, Toast, Flex } from "antd-mobile-rn";
...
let GameDB;
class FishGameList extends React.Component {
  constructor(props) {
    super(props);
  ...
  PlayGame(gameid, gametype, img, item) {
    console.log('PlayGame gameid',gameid)
    console.log('PlayGame gametype',gametype)
    if (isGameLock == true) {
      Toast.fail(
        "เรียนท่านสมาชิก เนื่องจากเราพบความผิดปกติในบัญชีของคุณ เราจึงไม่สามารถให้บริการคุณได้ชั่วคราว ถ้าคุญมีคำถามเพิ่มเติม โปรดติดต่อเจ้าหน้าที่บริการลูกค้า",
        2
    ...
    if(gametype==="JIF"){
      UMonEvent("Game", "Launch", `${item.gameName}_JiLiFishing`);   //Piwki 追蹤 加薪遊戲 記得添加
    }else if(gametype==="SPG"){
      UMonEvent("Game", "Launch", `${item.gameName}_SGFishing`);   //Piwki 追蹤 加薪遊戲 記得添加
    } 
    Toast.loading("เริ่มล็อบบี้เกม...", 200);
    ...
    fetchRequest(ApiPort.Game + gameid + "?isDemo=false&", "POST", data)
      .then(data => {
        ...
        if (data.errorCode == 0) {
          ...
          //   console.log('橫屏遊戲刪除頭部')
          //   this.navigateToScene("SGPGamePage", data.gameUrl, gametypes,BlegameReH);
          //   return;
          // }else {
          //    this.navigateToScene("GamePage", data.gameUrl, gametypes,BlegameReH);
          //  }
          console.log('first',data.gameUrl)
  ...
  renderRow = data => {
    ...
    return (
      ...
      >
        ...
        {item.provider == "JIF" || item.provider == "SPG" && (
            <View style={styles.imgTextBox}>

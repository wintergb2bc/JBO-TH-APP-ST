...
class preferentialPage extends React.Component {
	...
	render() {
		...
		const { promotionDetail, postTitle } = preferential
		let isHaveBtn = false
		if(promotionDetail) {
		...
		return <View style={{ backgroundColor: '#171717', flex: 1 }}>
			...
			>
				<WebViewIOS
					...
					style={{ width, position: 'relative', zIndex: 1000, flex: 1, marginBottom: isHaveBtn ? 140 : 110 }}
					files={[{
						href: 'cssfileaddress',
						type: 'text/css',
						rel: 'stylesheet'
			...
			{Platform.OS === 'android'&&
				...
				>
					<WebView
						mediaPlaybackRequiresUserAction={false}
						style={{ width, position: 'relative', zIndex: 1000, flex: 1, marginBottom: isHaveBtn ? 140 : 60 }}
						files={[{
							href: 'cssfileaddress',
							type: 'text/css',
							rel: 'stylesheet'

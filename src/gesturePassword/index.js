...
export default class GesturePassword extends Component {
    ...
    render() {
        let color = this.props.status === 'wrong' ? this.props.wrongColor : this.props.status === 'normal' ? this.props.normalColor : this.props.rightColor;

        return (
            <View style={[styles.frame, this.props.style, {flex: 1}]}>
                <View style={styles.message}>
                    <Text style={[styles.msgText, this.props.textStyle]}>
    ...
    renderCircles() {
        ...
        this.state.circles.forEach(function(c, i) {
            ...
            color = status === 'wrong' ? wrongColor : status === 'normal' ? normalColor : rightColor;
            inner = !!innerCircle;
            outer = !!outerCircle;
    ...
    renderLines() {
        let array = [], color;
        let { status, wrongColor, rightColor, transparentLine,normalColor } = this.props;
